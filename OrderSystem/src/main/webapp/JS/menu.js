        ...
        success : function(data) {
            data.forEach(function(food) {
                menuModel.foods.push({
                    ...
                    foodPicture : ko.observable(food.foodPicturePrefix + food.foodPicture),
                    add : function() {
                        ...
                        initModal("加入购物车", "添加<span class='text-info'>" + this.foodName() + "</span>成功");
                    },
                });
            });
        }
...
menuModel.order = function() {
    ...
    $.ajax({
        url : "getUserOrders.action?customerOrOrderName=" + window.userName,
        type : 'POST',
        success : function(data) {
            var orders = [];
...
menuModel.takeOrder = function() {
    ...
    var orders = [];
    menuModel.cartLines().forEach(function(line) {
        var foodPicturePrefix = line.linePicture.split('base64,')[0] + 'base64,';
        var foodPicture = line.linePicture.split('base64,')[1];
        orders.push({
            lineName : line.lineName,
            linePrice : line.linePrice,
            customerName : window.userName,
